...
interface KitchenViewModalProps {
    order: IOrder;
}

export const KitchenViewModal = ({ order }: KitchenViewModalProps) => {
    const { isOpen, open, close, products, isLoading, isReadyToServe, isUpdatingStatus, markReadyToServe } = useKitchenViewModal(order.id);
    return (
        <>
            <button
                        ...
                        <div className="flex items-center justify-between px-5 py-4 border-b border-stone-100 shrink-0">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-stone-400">Vista de cocina</p>
                                </div>
                            </div>
                            <button
                                onClick={close}
                            ...
                            {isLoading ? (
                                ...
                                products.map((item, idx) => (
                                    <div key={item.id ?? idx} className="flex gap-3 p-3 rounded-xl bg-stone-50 border border-stone-100">
                                        <div className="w-7 h-7 rounded-lg bg-orange-100 flex items-center justify-center shrink-0 text-xs font-bold text-orange-700">
                                            {item.cantidad}
                                        </div>
