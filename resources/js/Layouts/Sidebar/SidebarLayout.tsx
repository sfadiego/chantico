...
const SidebarLayout = ({
    ...
    const [showSelectedProductModal, setshowSelectedProductModal] = useState({
        productId: 0,
        descuento: 0,
    });
    ...
    const setShowProduct = (show: boolean) =>
        setshowSelectedProduct({ ...showSelectedProduct, showDetail: show });

    const setProductDiscount = (productId: number) => {
        const product = productsInOrder
            .filter((product) => product.producto_id === productId)
            .shift();
        const descuento = product ? product.descuento : 0;
        setshowSelectedProductModal({
    ...
    const [showDiscountProductModal, setShowDiscountProductModal] =
        useState(false);
    const [showCalculatePayModal, setShowCalculatePayModal] = useState(false);
    const handlePay = () => setShowCalculatePayModal(true);
    const disabledPay =
        estatus_pedido_id == OrderStatusEnum.Closed || !productsInOrder.length;
